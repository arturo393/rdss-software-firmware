...
const Status = (props) => {
  ...
  useEffect(() => {
    ...
    props.monitorData?.map((monitor) => {
      ...
      if (
        state.status.some((s) => s.id == parseInt(data.id)) &&
  ...
  return (
    <>
      <div className="col-md-12 text-center">
        <div className="row col-md-2">
          <span className="badge rounded-pill bg-primary">Connect</span>
          <span className="badge rounded-pill bg-light text-dark">
            {state.status.connected}
          </span>
        </div>
        <div className="row col-md-2">
          <span className="badge rounded-pill bg-primary">In Network</span>
          <span className="badge rounded-pill bg-light text-dark">
            {state.status.network}
          </span>
        </div>
        <div className="row col-md-2">
          <span className="badge rounded-pill bg-primary">With Alert</span>
          <span className="badge rounded-pill bg-light text-dark">
            {state.status.alert}
          </span>
        </div>
    ...
    // <Container className="darkBgColor">
    //   hola
    //   <Card>
