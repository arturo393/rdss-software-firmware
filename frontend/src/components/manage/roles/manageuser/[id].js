...
const ManageUser = (props) => {
    ...
    useEffect(() => {
        ...
        axios.post('http://localhost:3000/api/manage/rolid', rolId)
            .then((result) => {
                ...
                setState(prevState => ({ 
                    ...prevState, rol: objRol 
                }))

                setState(prevState => ({ 
                    ...prevState, userRol: objRol.users
                }))
    ...
    const handleChange = (e) => {
        if (e.target.checked){
            ...
            setState(prevState => ({ 
                ...prevState, userRol: state.userRol.concat(newUserRol)
            }))
        }
        else{
            const usersRol = state.userRol?.filter(i=> i.name !== e.target.name)
            setState(prevState => ({ 
                ...prevState, userRol: usersRol
            }))
        }
    }
    ...
    const onSubmit = (e) => {
        ...
        axios.post('http://localhost:3000/api/manage/editRolUser', rolUser)
            .then((result) => {
